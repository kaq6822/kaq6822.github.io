...
- 블로그 하위 페이지(목록·글)의 헤더는 메인 페이지의 섹션 목차(소개/기술/…)를 두지 않습니다. 이 링크들은 랜딩 페이지로 이탈시켜 혼란을 주므로, 헤더에는 `<header class="site-header--minimal">` 아래 로고(홈)와 우측의 `블로그`·`GitHub` 버튼(`.nav-actions`)만 둡니다. 햄버거/드로어(`.nav-toggle`, `.nav-panel` 드로어)와 관련 `<noscript>` 폴백은 사용하지 않습니다.
...
- 커밋 메시지는 [Conventional Commits](https://www.conventionalcommits.org/ko/v1.0.0/)를 따릅니다: `<type>(<scope>): <제목>`.
- 제목은 한국어 명령문으로 작성하고 마침표를 붙이지 않습니다 (예: `feat: 연락처 섹션 추가`, `fix(nav): 모바일 내비게이션 겹침 수정`).
- 사용하는 `type`:
  - `feat` — 페이지·섹션·기능 추가
  - `fix` — 레이아웃 깨짐, 링크 오류, 빌드 오류 수정
  - `content` — 포트폴리오 문구·경력·프로젝트 설명 등 콘텐츠 변경
  - `post` — 블로그 글 추가 및 수정
  - `style` — 시각적 스타일 조정 (기능 변화 없음)
  - `refactor` — 결과물 변화 없는 마크업/CSS/스크립트 정리
  - `docs` — `AGENTS.md`, `CLAUDE.md`, `TODO.md` 등 문서 변경
  - `chore` — 설정 파일(`_config.yml`, `robots.txt` 등) 및 기타 잡무
- `scope`는 선택이며, 쓸 경우 변경 범위를 나타내는 소문자 한 단어를 씁니다 (예: `blog`, `nav`, `hero`, `seo`).
- 본문이 필요하면 제목과 빈 줄로 구분하고, 무엇을 왜 바꿨는지 한국어로 적습니다.
